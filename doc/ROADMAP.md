...
*Numbers last changed: 2026-09-07. Re-run `omacar roadmap` any time — it rewrites this file only when a number actually moved.*
...
| | Files | Lines |
|---|---:|---:|
| Python — the whole diagnostic side | 49 | 20,155 |
| JavaScript — the app | 44 | 13,433 |
| CSS | 7 | 2,665 |
| QML — the Quickshell plugin | 6 | 3,132 |
| Shell — the CLI and the installer | 5 | 1,281 |
| Tests | 7 | 3,047 |
| Documentation | 10 | 2,981 |
| Data — codes, resets, procedures, profiles | 6 | 1,376 |
| **Tracked in git, all of it** | | **48,932** |
...
**Shipped** — 76 commits on `omacar-launch`. The most recent, unedited:

- `2026-09-07` Every agent on this machine knows what is plugged in
- `2026-09-07` The car, as tools any agent harness can call
- `2026-09-07` The screen reflects the mode; it does not decide it
...
- Flagship scan tools run from the low thousands to well over ten thousand dollars, plus an annual subscription. Indicative, not quoted from a price list.
  <br>*Asserted 2026-09-02, 5 days ago. To re-check: Check a current dealer or distributor price for a Zeus- or Apollo-class tablet and its renewal. The argument depends on the ratio, not the figure, so a wrong number is embarrassing rather than load-bearing -- but it should still be right.*
- A full sweep of the manufacturer identifier range is about seventy minutes per module.
  <br>*Asserted 2026-09-02, 5 days ago. To re-check: Arithmetic: the identifier count above, at the pacing lib/autodisc.py actually uses. Time one real burst with `omacar discover` and divide.*
- Pre-CAN protocol support (J1850, ISO 9141-2, KWP2000) is written from the ELM327 datasheet and the ISO documents and has never been run against a pre-CAN car.
  <br>*Asserted 2026-09-02, 5 days ago. To re-check: Run `omacar doctor` on a pre-1996-protocol vehicle. The moment somebody does, this claim and roadmap item 2.5 both change, and every protocol entry that says 'untested' has to be re-read.*
...
- Generic mode 01 PID 0x5B, hybrid battery remaining life, appears in this car's own supported-PID bitmap and has never once been polled.
  <br>*Asserted 2026-09-02, 5 days ago. To re-check: One request. A support bitmap is a claim the ECU makes about itself, not a reading, and bitmaps overstate. Ask it before anything is built on it.*
