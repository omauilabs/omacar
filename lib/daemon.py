...
def main():
    ...
    def hand_over():
        """Close the port so a one-off command can have it, then take it back."""
        publish({"connected": False, "status": "yielded", "port": port,
                 "note": "a command is using the adapter"})
        if conn is not None:
            try:
        ...
        while True:
            ...
            if now - last_row >= 1.0:
                try:
                    ...
                    # The gauge outranks the record. A row that cannot be
                    # written — a full disk, or the readonly-database error
                    # try_connect() warns about — used to end the process and
                    # take the live reading down with it, which is the wrong way
                    # round: the person is watching a number, not a database.
                    # Said once, because at one row a second a broken record
                    # would otherwise write the log file the disk has no room
                    # for.
                    if not db_broken:
